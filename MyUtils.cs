...
internal class MyUtils
{
    ...
    public static void ApplyEquipFunctional(Player plr, Item item)
    {
        ...
        var itemDamage = plr.GetWeaponDamage(item);
        var itemCrit = plr.GetWeaponCrit(item);
        var tileRangeX = Player.tileRangeX;
        var tileRangeY = Player.tileRangeY;
        ...
        if (item.type == 861)
        {
            plr.accMerman = true;
            plr.wolfAcc = true;
            if (plr.hideVisibleAccessory[itemSlot])
            {
                plr.hideMerman = true;
                plr.hideWolf = true;
            }
        ...
        if ((!plr.mount.Active || plr.mount.Type != 47) && !plr.hideVisibleAccessory[itemSlot] && (item.type == 4822 || item.type == 4874))
        {
        ...
        if (item.wingSlot > 0)
        {
            if (!plr.hideVisibleAccessory[itemSlot] || (plr.velocity.Y != 0f && !plr.mount.Active))
            {
                plr.wings = item.wingSlot;
            }
            plr.wingsLogic = item.wingSlot;
            //plr.equippedWings = item;
        }
